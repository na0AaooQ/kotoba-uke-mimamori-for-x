...
`chrome.storage.local` に保存してよいのは、ユーザーが明示的に選択した設定値のみです。現在のMVP土台では、拡張機能の有効 / 無効のような最小限の設定値に限定してください。
...
設定変更のリアルタイム反映は、既存DOM状態・ぼかし状態・ワンクッションUI状態を安全に扱う必要があるため、別PRで慎重に検討してください。
