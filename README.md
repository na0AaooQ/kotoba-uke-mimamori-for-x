...
`docs/privacy.html` と `docs/manual.html` では、外部送信を行わないこと、保存対象が `enabled` と `cushionSensitivity` のみであること、ON/OFFおよび表示されやすさの反映方法やワンクッションUIの動作を説明しています。また、3ページに目次を追加し、共通レイアウトとフッター表記を調整しています。
