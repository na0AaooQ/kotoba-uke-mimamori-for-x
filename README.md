...
自己常時ON試用で得た「ワンクッションUIの表示されやすさを自分で変更したい」という改善候補について、MVPに追加する場合の設計案を整理します。本節は実装前の設計メモであり、現時点では設定値、オプション画面、判定処理、privacy / manual ページの動作を変更しません。現在の標準しきい値 `DEFAULT_CUSHION_THRESHOLD = 80`、初期設定 `enabled=false`、開発用フラグがいずれも `false` である状態を維持します。
...
`high` は心に負荷がかかる可能性のある表現へ反応する範囲を広げる案であり、健全な批判・異論・反対意見を対象にする方針への変更ではありません。実装後の初期値は `standard` とし、未設定時にも現在の挙動が変わらないようにします。

### 保存する設定値案

保存キーは、何の感度かが読み取れる `cushionSensitivity` を採用候補とします。`sensitivity` よりも、将来ほかの設定が増えた際に意味が明確です。
...
実装する場合も `chrome.storage.local` に保存する対象はこの2つのユーザー設定値だけに限定します。`cushionSensitivity` が未設定または不正値の場合は `standard` に正規化します。現在の実装で保存される設定値は引き続き `enabled` のみであり、本設計メモの追加によって保存対象は増えません。
...
- `high` で `score` が60〜79の場合、内部の `riskLevel` が `medium` のまま `shouldCushion=true` になり得るため、表示可否は `shouldCushion` を基準に扱い、実装時にテストで確認する。

`settings.js`:

- `DEFAULT_SETTINGS` に `cushionSensitivity: 'standard'` を追加する案とする。
...
- 数値入力ではなく、説明文を併記できるラジオボタンを第一候補とする。
- 表示名は `ワンクッションの表示されやすさ` とし、`少なめ`、`標準`、`多め` の意味をやさしい文言で説明する。
- 初期表示は `標準` とし、保存時は既存の保存完了メッセージのトーンに合わせる。
- 実装時にユーザー向け文言を追加する場合は、`_locales/ja/messages.json` と `_locales/en/messages.json` を同時に更新する。
...
感度設定を実装するPRでは、`docs/privacy.html` に保存対象の追加を明記します。追記内容は、保存する設定値が `enabled` と `cushionSensitivity` であること、および投稿本文、判定結果、閲覧履歴、投稿URL、ユーザー名、アカウントID、`score`、`matchedRules`、`categories`、`reasons` を保存しないことです。投稿本文や判定結果を外部送信しない方針も維持します。文言は実装時の説明案であり、法務文言を最終確定するものではありません。

同じPRで `docs/manual.html` に、オプション画面から表示されやすさを選べること、`少なめ` / `標準` / `多め` の意味、変更後は開いているXページを再読み込みすると反映されることを追記します。

### 実装時のテスト方針

本設計メモのみのPRではテストコードを追加しません。感度設定を実装するPRでは、既存テストに以下を追加します。
