...
オプション画面の `enabled` 設定を `content.js` の処理へ接続しています。初期値は `enabled: false` で、ユーザーが明示的にONにした場合のみ、通常判定によるワンクッション処理が有効になります。

ON/OFFの変更は、現時点では開いているXページを再読み込みすると反映されます。リアルタイム反映は今後検討予定です。保存する設定値は `enabled` のみに限定し、投稿本文・判定結果・URL・ユーザー情報は保存しません。
...
`content.js` は、オプション画面で保存された `enabled` 設定を読み込み、`enabled=true` の場合のみX上の投稿DOM候補を検出し、投稿本文らしきテキストを抽出します。`risk-detector.js` との接続では、投稿本文をログに出さず、保存・外部送信も行わず、判定対象件数とワンクッション候補件数のみで処理状況を確認します。`shouldCushion=true` の投稿候補にはワンクッションUIとぼかし表示を適用します。`enabled=false` ではDOM監視・通常判定・候補属性付与・ワンクッションUI・ぼかし表示を行いません。
...
- ON/OFFの変更は、開いているXページを再読み込みすると反映される
...
12. `chrome.storage.local` に保存されている値が `enabled` のみであることを確認する。
...
`chrome.storage.local` に保存されるのは `enabled` のみであることを確認します。投稿本文、判定結果、URL、ユーザー情報、`score`、`matchedRules`、`categories`、`reasons` は保存しません。
...
- [ ] `chrome.storage.local` に `enabled` 以外が保存されない
...
- [ ] `chrome.storage.local` に `enabled` 以外が保存されていない
